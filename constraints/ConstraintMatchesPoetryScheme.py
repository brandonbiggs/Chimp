    ...
    def is_satisfied_by_state(self, phrase: str) -> bool:
        ...
        for word in phrase.split(" "):
            try:
                ...
                self.stress_pattern_dict[word] = output
            except:
                return False

        if self.must_match:
            matching_word = phrase.split(" ")[self.stress_pattern_word_position]
            matching_pattern = self.stress_pattern_dict[matching_word]
            if matching_pattern != self.stress_pattern:
                return False
        if self.check_pattern:
            previous_stress = None
            for key,value in self.stress_pattern_dict.values():
