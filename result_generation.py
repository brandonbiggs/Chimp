...
def graph(
    ...
    for letter in ascii_lowercase:
        chimp_sentences = tongue_twister_chimp(
            letter, pickle_file, sentence_length, iterations
        )
        markov_sentences = tongue_twister_markov_model(
        ...
        results += (
            ...
            + str(chimp_sentences)
            + "\nMarkov Sentences: "
            + str(markov_sentences)
            + "\n"
        )

    counts = (
        "Total Chimp Counts: "
        + str(chimp_sentences_total)
        + "\nTotal Markov Counts: "
        + str(markov_sentences_total)
        + "\nChimp Average: "
        + str(array_average(chimp_sentences_total))
        + "\nMarkov Average: "
        + str(array_average(markov_sentences_total))
    )
...
def graph_two():
    ...
    sentence_length = [2, 4, 6, 8, 10, 12, 14, 16]
    ...
    for number in number_of_sentences:
        ...
        for length in sentence_length:
            ...
            for letter in ascii_lowercase:
                chimp_sentences = tongue_twister_chimp(
                    letter, pickle_file, length, iterations
                )
                markov_sentences = tongue_twister_markov_model(
                    letter, pickle_mm_file, length, iterations
                )
                chimp_sentences_total.append(chimp_sentences)
                markov_sentences_total.append(markov_sentences)
