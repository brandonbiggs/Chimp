...
class parallel_nhmm:
    ...
    def process(self) -> None:
        ...
        queue = Queue()
        threads = []
        for x in range(self.layers - 1, -1, -1):
            t = threading.Thread(target=self.parallel_observed_nodes, args=(x, queue,))
            t.daemon = True
            # threads.append(t)
            t.start()
        # Documentation says I need this line, but if I do, it pauses indefinite.
        # queue.join()
        for x in range(queue.qsize()):
            item = queue.get()
            self.observed_node_betas[item[0]] = item[2][1]
            self.constrained_observed_emission_probabilities[item[0]] = item[1]
